			...
			@if($posts->count())
				@foreach(array_chunk($posts->getCollection()->all(), 3) as $row)
					...
					<div class="posts-row-wrap">
						<div class="posts-row"> <!-- -->
							@foreach($row as $post)
							<article class="article-blocks" onclick="window.location.href='{{ URL::action('post-show', $post->id) }}'">
							    @if(Auth::check())
							    ...
							    <div class=""> <p class="published-on">Posted {{ $post->created_at->diffForHumans()}}</p> </div>
							 	...
							 	<div class="post-title-header"> <a class="post-title-thumbnail" href="{{ URL::action('post-show', $post->id) }}">{{ $post->title }} </a> </div>

							 	<div class="post-body">{{ Markdown::parse(Str::limit($post->body, 70)) }}</div>
							 	<div> <a class="read-more" href="{{ URL::action('post-show', $post->id) }}"> more &rarr;</a></div>
							</article>
							@endforeach
